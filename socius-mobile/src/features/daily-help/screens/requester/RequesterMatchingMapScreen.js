...
const MatchingMapScreen = ({ navigation, route }) => {
  ...
  /** Progress notifications are owned globally (see activeHelpSessionSync); push a refresh when meeting state changes here */
  useEffect(() => {
    ...
    if (!active || !isRequester || !request.sessionEndsAt) return undefined;
