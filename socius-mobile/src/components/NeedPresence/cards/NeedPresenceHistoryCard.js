...
const NeedPresenceHistoryCard = ({ item, onAction }) => {
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <View style={[styles.avatarPlaceholder, {
          ...
          <Icon name="eye-check" size={scale(28)} color="#DC5C69" />
        </View>
      ...
      <View style={{ marginTop: vscale(8), paddingTop: vscale(8), borderTopWidth: 1, borderTopColor: '#F1F5F9', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text style={{ fontSize: ms(9), color: '#94A3B8', marginRight: spacing(4) }}>TYPE:</Text>
          <Text style={{ fontSize: ms(11), color: '#1E293B', fontWeight: '600' }}>{item.category || 'General Presence'}</Text>
        </View>
        ...
        >
          <Text style={[
            ...
            {statusStyle.label === 'Active' ? 'View Details' : 'Request Again'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
